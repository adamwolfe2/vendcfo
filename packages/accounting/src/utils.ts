import { decryptOAuthState, encryptOAuthState } from "@vendcfo/encryption";
 ...
 *
 * @param transactionId - The VendCFO transaction ID
 ...
 * // Returns: "vendhub-tx-tx-123-job-456"
 */
export function generateTransactionIdempotencyKey(
  transactionId: string,
  jobId: string,
): string {
  return `vendhub-tx-${transactionId}-${jobId}`;
}
 ...
 * // Returns: "vendhub-attachment-abc-123-receipt.pdf"
 */
export function generateAttachmentIdempotencyKey(
  providerTransactionId: string,
  fileName: string,
): string {
  return `vendhub-attachment-${providerTransactionId}-${fileName}`;
}

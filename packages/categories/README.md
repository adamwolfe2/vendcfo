...
```bash
npm install @vendcfo/categories
```
...
```typescript
import { CATEGORIES, getCategoryBySlug, getParentCategory } from '@vendcfo/categories';
...
```typescript
import { getTaxRateForCategory, getTaxTypeForCountry } from '@vendcfo/categories';
...
```typescript
import { getCategoryColor, CATEGORY_COLOR_MAP } from '@vendcfo/categories';

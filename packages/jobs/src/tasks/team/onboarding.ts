...
import { GetStartedEmail } from "@vendcfo/email/emails/get-started";
import { TrialEndedEmail } from "@vendcfo/email/emails/trial-ended";
import { TrialExpiringEmail } from "@vendcfo/email/emails/trial-expiring";
import { WelcomeEmail } from "@vendcfo/email/emails/welcome";
import { render } from "@vendcfo/email/render";
import { createClient } from "@vendcfo/supabase/job";
...
export const onboardTeam = schemaTask({
  ...
  run: async ({ userId }) => {
    ...
    await resend.emails.send({
      to: user.email,
      subject: "Welcome to VendCFO",
      from: "Pontus from VendCFO <pontus@vendcfo.ai>",
      html: await render(
        WelcomeEmail({
    ...
    if (await shouldSendEmail(user.team_id)) {
      await resend.emails.send({
        from: "Pontus from VendCFO <pontus@vendcfo.ai>",
        to: user.email,
        subject: "Get the most out of VendCFO",
        html: await render(
          GetStartedEmail({
            fullName: user.full_name,
          }),

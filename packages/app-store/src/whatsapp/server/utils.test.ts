...
describe("extractInboxIdFromMessage", () => {
  describe("Pattern 1: 'Connect to VendCFO: xxx'", () => {
    test("should extract inbox ID from standard format", () => {
      expect(extractInboxIdFromMessage("Connect to VendCFO: abc123")).toBe(
        "abc123",
      );
    });

    test("should extract inbox ID with case-insensitive matching", () => {
      expect(extractInboxIdFromMessage("connect to vendhub: xyz789")).toBe(
        "xyz789",
      );
    ...
    test("should extract inbox ID with leading/trailing whitespace", () => {
      ...
      expect(extractInboxIdFromMessage("\tConnect to VendCFO: abc123\n")).toBe(
        "abc123",
      );
    });
    ...
    test("should handle longer inbox IDs", () => {
      expect(
        extractInboxIdFromMessage("Connect to VendCFO: abc123def456ghi789"),
  ...
  describe("Edge cases and invalid inputs", () => {
    ...
    test("should return null for messages with special characters", () => {
      expect(extractInboxIdFromMessage("Connect to VendCFO: abc-123")).toBe(
        "abc",
      ); // Only matches up to special char
      expect(extractInboxIdFromMessage("Connect to VendCFO: abc_123")).toBe(
        "abc",
      );
  ...
  describe("Real-world scenarios", () => {
    test("should handle the exact format generated by the client", () => {
      ...
      const message = `Connect to VendCFO: ${inboxId}`;
    ...
    test("should prioritize Pattern 1 over Pattern 3", () => {
      ...
      expect(extractInboxIdFromMessage("Connect to VendCFO: abc")).toBe("abc");
    });

    test("should handle messages with extra text before pattern", () => {
      expect(extractInboxIdFromMessage("Hi! Connect to VendCFO: abc123")).toBe(
        "abc123",
      );
    });
    ...
    test("should handle messages with extra text after pattern", () => {
      ...
      expect(
        extractInboxIdFromMessage("Connect to VendCFO: abc123 thanks!"),
      ).toBe("abc123");
    });
  });
});

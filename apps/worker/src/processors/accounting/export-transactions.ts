import { ACCOUNTING_ERROR_CODES, RATE_LIMITS } from "@vendcfo/accounting";
import {
...
interface CategorizedTransaction {
  ...
  toSyncAttachments: Array<{
    ...
    removedAttachments: Array<{ vendhubId: string; providerId: string | null }>;
  }>;
  ...
  private categorizeTransactions(
    ...
    for (const tx of transactions) {
      ...
      const removedAttachments = [...syncedIds]
        ...
        .map((vendhubId) => ({
          vendhubId,
          providerId: syncedMapping[vendhubId] ?? null,
        }));

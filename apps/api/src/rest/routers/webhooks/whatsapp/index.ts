...
} from "@vendcfo/app-store/whatsapp/server";
import { WhatsAppAlreadyConnectedToAnotherTeamError } from "@vendcfo/db/errors";
import {
...
import { getTeamById, getTeamByInboxId } from "@vendcfo/db/queries";
  ...
  try {
    if (action === "confirm") {
      ...
      await confirmSuggestedMatch(db, {
        ...
        inboxId,
        transactionId,
        userId: undefined, // WhatsApp interactions don't have VendCFO user ID mapping
        teamId,
      });

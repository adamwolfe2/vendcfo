...
import config from "@vendcfo/app-store/fortnox";
...
app.openapi(
  ...
  async (c) => {
    ...
    try {
      ...
      logger.error("Fortnox OAuth: failed to decrypt state", { error: err });
      return c.redirect(
        buildErrorRedirect(dashboardUrl, "invalid_state", "fortnox"),
        302,
      );
    }

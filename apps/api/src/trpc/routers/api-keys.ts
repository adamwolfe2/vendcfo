...
import { apiKeyCache } from "@vendcfo/cache/api-key-cache";
import {
  deleteApiKey,
  getApiKeysByTeam,
  upsertApiKey,
} from "@vendcfo/db/queries";
import { ApiKeyCreatedEmail } from "@vendcfo/email/emails/api-key-created";
    ...
    .mutation(async ({ ctx: { db, teamId, session, geo }, input }) => {
      ...
      if (data) {
        try {
          ...
          resend.emails.send({
            from: "VendCFObot <vendhubbot@vendcfo.ai>",

...
// Mock @vendcfo/db/queries with a Proxy to handle any export
...
mock.module("@vendcfo/db/queries", () => dbQueriesMock);

// Mock @vendcfo/supabase/storage
mock.module("@vendcfo/supabase/storage", () => ({
  signedUrl: mocks.signedUrl,
  remove: mock(() => Promise.resolve({ error: null })),
}));

// Mock @vendcfo/job-client
mock.module("@vendcfo/job-client", () => ({
...
// Mock @vendcfo/import
mock.module("@vendcfo/import", () => ({
  formatAmountValue: mocks.formatAmountValue,
}));

// Mock @vendcfo/invoice
mock.module("@vendcfo/invoice/calculate", () => ({
...
mock.module("@vendcfo/invoice/utils", () => ({
...
mock.module("@vendcfo/invoice/token", () => ({
  verify: mock(() => ({ id: "invoice-123", teamId: "test-team-id" })),
}));

mock.module("@vendcfo/invoice", () => ({
  DEFAULT_TEMPLATE: {},
}));
...
// Mock @vendcfo/db/client
mock.module("@vendcfo/db/client", () => ({
  db: mockDb,
  primaryDb: mockDb,
  connectDb: mock(async () => mockDb),
}));

// Mock @vendcfo/cache/team-cache
mock.module("@vendcfo/cache/team-cache", () => ({
  teamCache: {

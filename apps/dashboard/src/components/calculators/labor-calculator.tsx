...
export function LaborCalculator() {
  ...
  const updateEmployee = (id: string, field: keyof Omit<Employee, "id">, value: string) => {
    setEmployees((prev) =>
      prev.map((e) =>
        e.id === id
          ? { ...e, [field]: field === "name" ? value : (parseFloat(value) || 0) }
          : e
      )
    );
  };
  ...
  const handleSharedChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setShared((prev) => ({ ...prev, [name]: parseFloat(value) || 0 }));
  };

  // Per-employee costs
  const employeeCosts = employees.map((emp) => {
    const weeklyCost = emp.hourlyWage * emp.hoursPerWeek * (1 + shared.benefitsPct / 100);
    const monthlyCost = weeklyCost * 4.33;
    return { ...emp, weeklyCost, monthlyCost };
  });

  // Totals
  const totalMonthlyCost = employeeCosts.reduce((sum, e) => sum + e.monthlyCost, 0);
  const totalStops = shared.routesServiced * shared.avgStopsPerRoute;
  const costPerRoute = shared.routesServiced > 0 ? totalMonthlyCost / shared.routesServiced : 0;
      ...
      <CardContent>
        ...
        <div className="space-y-3 mb-5">
          {employees.map((emp, idx) => (
            <div key={emp.id} className="rounded-md border border-border bg-secondary/50 p-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateEmployee(emp.id, "hourlyWage", e.target.value)}
          ...
          {employees.length >= MAX_EMPLOYEES && (
            <p className="text-xs text-muted-foreground text-center">Maximum of {MAX_EMPLOYEES} employees reached.</p>
          )}
        </div>
        ...
        <div className="space-y-4 mb-5">
          <div>
            ...
            <Input type="number" id="benefitsPct" name="benefitsPct" value={shared.benefitsPct} onChange={handleSharedChange} className="mt-1" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="routesServiced">Routes Serviced</Label>
              <Input type="number" id="routesServiced" name="routesServiced" value={shared.routesServiced} onChange={handleSharedChange} className="mt-1" />
            </div>
            <div>
              <Label htmlFor="avgStopsPerRoute">Avg Stops/Route</Label>
              <Input type="number" id="avgStopsPerRoute" name="avgStopsPerRoute" value={shared.avgStopsPerRoute} onChange={handleSharedChange} className="mt-1" />
            </div>
          </div>
        </div>
        ...
        {employeeCosts.length > 1 && (
          <div className="bg-secondary/50 p-4 rounded-lg border border-border mb-4">
            <h3 className="text-sm font-bold text-foreground uppercase tracking-wider mb-3">Per-Employee Costs</h3>
            <div className="space-y-2 font-mono text-sm">
              {employeeCosts.map((emp) => (
                <div key={emp.id} className="flex items-center justify-between">
                  <span className="text-muted-foreground truncate mr-2">{emp.name}</span>
                  <div className="flex gap-4 shrink-0">
                    <span className="text-muted-foreground text-xs">${emp.weeklyCost.toFixed(0)}/wk</span>
                    <span className="font-medium text-foreground">${emp.monthlyCost.toFixed(2)}/mo</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-secondary p-4 rounded-lg border border-border">
          ...
          <div className="space-y-3 font-mono text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-bold text-foreground">${totalMonthlyCost.toFixed(2)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Cost / Route:</span>
              <span className="text-foreground">${costPerRoute.toFixed(2)}</span>
            </div>
            <div className="flex justify-between pt-2 border-t border-border">
              <span className="text-muted-foreground">Cost / Stop:</span>
              <span className="font-medium text-foreground">${costPerStop.toFixed(2)}</span>
            </div>
          </div>
        </div>
      </CardContent>

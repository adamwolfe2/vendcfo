...
export function ProductMixCalculator() {
  ...
  const weightedMargin =
    totalUnits > 0
      ? products.reduce((s, p) => s + (p.unitsSold / totalUnits) * p.margin, 0)
      : 0;
  ...
  return (
    <Card className="w-full max-w-md">
      <CardHeader className="pb-4">
        <CardTitle>Product Mix Analyzer</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-3 mb-6">
            ...
            <div key={i} className="grid grid-cols-3 gap-2">
              ...
              <div>
                ...
                <Input
                  type="number"
                  value={p.unitsSold}
                  onChange={(e) => handleChange(i, "unitsSold", e.target.value)}
                  className="mt-1"
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  value={p.margin}
                  onChange={(e) => handleChange(i, "margin", e.target.value)}
                  className="mt-1"
                />
              </div>
            </div>
          ...
          <button
            onClick={() =>
              setProducts([...products, { name: "", unitsSold: 0, margin: 0 }])
            }
        ...
        <div className="bg-secondary p-4 rounded-lg border border-border">
          ...
          <div className="space-y-3 font-mono text-sm">
            ...
            <div className="flex justify-between pt-2 border-t border-border">
              <span className="text-muted-foreground">Weighted Margin:</span>
              <span
                className={`font-bold ${weightedMargin < 35 ? "text-red-500" : "text-green-600"}`}
              >

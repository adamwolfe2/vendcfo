...
export function RevShareCalculator() {
  ...
  const handleChange = (
    ...
    setInputs((prev) => ({
      ...prev,
      [name]: name === "revShareType" ? value : Number.parseFloat(value) || 0,
    }));
  };
    ...
    <Card className="w-full max-w-md">
      ...
      <CardContent>
        <div className="space-y-4 mb-6">
          <div>
            <Label htmlFor="rs-grossSales">Monthly Gross Sales ($)</Label>
            <Input
              type="number"
              id="rs-grossSales"
              name="grossSales"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={handleChange}
                className="w-full mt-1 h-9 rounded-md border border-border bg-background px-3 text-sm text-foreground"
              >
          ...
          {inputs.revShareType === "tiered" && (
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Commission applies only above this amount.
              </p>
            </div>
          )}
        </div>
        <div className="bg-secondary p-4 rounded-lg border border-border">
          ...
          <div className="space-y-3 font-mono text-sm">
            ...
            <div className="flex justify-between">
              <span className="text-muted-foreground">Net Retained:</span>
              <span className="text-green-600">
                ${results.netRevenueRetainedByOperator.toLocaleString()}
              </span>
            </div>
            <div className="flex justify-between pt-2 border-t border-border">
              <span className="text-muted-foreground">Margin Risk:</span>
              <span
                className={`font-medium ${results.revShareAsPctOfGrossMargin > 30 ? "text-red-500" : "text-foreground"}`}
              >

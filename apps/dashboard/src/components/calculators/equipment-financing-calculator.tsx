...
export function EquipmentFinancingCalculator() {
  const [inputs, setInputs] = useState<EquipmentFinancingInputs>({
    equipmentPrice: 4500, downPayment: 500, apr: 8.5, termMonths: 36,
    expectedMonthlyGrossRevenue: 600, expectedMonthlyCogs: 240, expectedRevSharePct: 10, expectedMonthlyServicingCost: 75,
  });
  ...
  return (
    <Card className="w-full max-w-md">
      <CardHeader className="pb-4"><CardTitle>Equipment ROI & Financing</CardTitle></CardHeader>
      <CardContent>
        <div className="space-y-4 mb-6">
          <div className="grid grid-cols-2 gap-4">
            <div><Label htmlFor="ef-price">Machine Price ($)</Label><Input type="number" id="ef-price" name="equipmentPrice" value={inputs.equipmentPrice} onChange={handleChange} className="mt-1" /></div>
            <div><Label htmlFor="ef-down">Down Payment ($)</Label><Input type="number" id="ef-down" name="downPayment" value={inputs.downPayment} onChange={handleChange} className="mt-1" /></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div><Label htmlFor="ef-apr">Loan APR (%)</Label><Input type="number" id="ef-apr" name="apr" value={inputs.apr} onChange={handleChange} className="mt-1" /></div>
            <div><Label htmlFor="ef-term">Term (Months)</Label><Input type="number" id="ef-term" name="termMonths" value={inputs.termMonths} onChange={handleChange} className="mt-1" /></div>
          </div>
          <div className="grid grid-cols-2 gap-4 border-t border-border pt-3">
            <div><Label htmlFor="ef-rev">Target Rev / Mo</Label><Input type="number" id="ef-rev" name="expectedMonthlyGrossRevenue" value={inputs.expectedMonthlyGrossRevenue} onChange={handleChange} className="mt-1" /></div>
            <div><Label htmlFor="ef-cogs">Est COGS / Mo</Label><Input type="number" id="ef-cogs" name="expectedMonthlyCogs" value={inputs.expectedMonthlyCogs} onChange={handleChange} className="mt-1" /></div>
          </div>
        </div>

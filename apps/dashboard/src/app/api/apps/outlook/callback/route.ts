...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error("[Outlook Callback] Failed to decrypt state:", err);
    return NextResponse.redirect(
      `${dashboardUrl}/oauth-callback?status=error&error=invalid_state`,
    );
  }
  ...
  if (error || !code) {
    ...
    console.info("Outlook OAuth error or cancelled", { error, errorCode });
    ...
    return NextResponse.redirect(
      `${dashboardUrl}/inbox?connected=false&error=${errorCode}&provider=outlook`,
    );
  }
  ...
  try {
    ...
    console.info("Outlook inbox account created successfully", {
      teamId: parsedState.teamId,
    ...
    return NextResponse.redirect(
      `${dashboardUrl}/inbox?connected=false&error=token_exchange_failed&provider=outlook`,
    );
  }
}

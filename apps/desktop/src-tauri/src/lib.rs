...
async fn create_preloaded_search_window(
    ...
    .title("VendHub Search")
    .inner_size(720.0, 450.0)
    .min_inner_size(720.0, 450.0)
    .resizable(false)
    .user_agent("Mozilla/5.0 (compatible; VendHub Desktop App)")
    .transparent(true)
    .decorations(false)
...
fn get_app_url() -> String {
    ...
    match env.as_str() {
        ...
        "staging" => {
            let url = "https://beta.vendhub.com".to_string();
...
fn handle_deep_link_event(app_handle: &tauri::AppHandle, urls: Vec<String>) {
    for url in &urls {
        // Only handle vendhub schemes (vendhub://, vendhub-dev://, vendhub-staging://)
        if let Some(idx) = url.find("://") {
            let scheme = &url[..idx];
            if !scheme.contains("vendhub") {
                continue;
            }
            let path = &url[idx + 3..];
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Set the default app menu to restore the VendHub menu

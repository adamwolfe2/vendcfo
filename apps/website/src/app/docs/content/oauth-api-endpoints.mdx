...
description: Technical reference for VendHub OAuth 2.0 endpoints.
section: developer
order: 4
---

Complete technical reference for VendHub's OAuth 2.0 implementation. These endpoints follow the [OAuth 2.0 RFC 6749](https://tools.ietf.org/html/rfc6749) and [PKCE RFC 7636](https://tools.ietf.org/html/rfc7636) specifications.

## Base URLs

| Environment | URL |
|-------------|-----|
| Authorization | `https://vendcfo.vercel.app/oauth` |
| Token & API | `https://api.vendhub.com/v1` |
...
```
GET https://vendcfo.vercel.app/oauth/authorize
```
...
```
POST https://api.vendhub.com/v1/oauth/token
```
...
```
POST https://api.vendhub.com/v1/oauth/revoke
```
...
Once you have an access token, use the VendHub SDK for API requests:
...
const profit = await vendhub.metrics.profit({
  from: "2024-01-01",
  to: "2024-12-31",
});
```

See the [SDK documentation](https://github.com/vendhub-ai/vendhub-ts) for all available methods.

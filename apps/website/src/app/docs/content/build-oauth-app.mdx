...
description: Create applications that integrate with VendHub using OAuth 2.0.
section: developer
order: 2
---

Build applications that let users connect their VendHub accounts. OAuth apps can access financial data on behalf of users, enabling integrations, automations, and custom tools.

## What you can build

With OAuth, you can build:

- **Browser extensions** that display VendHub data
- **Mobile apps** with VendHub integration
...
- **Public apps** listed in the VendHub app directory

## How OAuth works

OAuth 2.0 lets your app request access to a user's VendHub data without handling their password:

1. Your app redirects users to VendHub's authorization page
2. Users log in and approve the requested permissions
3. VendHub redirects back with an authorization code
4. Your app exchanges the code for access tokens
5. Use access tokens to call the VendHub API

## Getting started

### Step 1: Create an OAuth application

1. Go to [Settings → Developer](https://vendcfo.vercel.app/settings/developer)
...
```
https://vendcfo.vercel.app/oauth/authorize?
  response_type=code&
...
After the user authorizes, VendHub redirects to your `redirect_uri`:
...
```bash
curl -X POST https://api.vendhub.com/v1/oauth/token \
...
```typescript
import { VendHub } from "@vendhub-ai/sdk";

const vendhub = new VendHub({
  token: "mid_at_xxxxx", // Access token from OAuth flow
});

// List transactions
const transactions = await vendhub.transactions.list({
  pageSize: 50,
});

// Get invoices
const invoices = await vendhub.invoices.list({
  pageSize: 20,
});

// Get financial metrics
const revenue = await vendhub.metrics.revenue({
  from: "2024-01-01",
  to: "2024-12-31",
});
```
...
```bash
curl -X POST https://api.vendhub.com/v1/oauth/revoke \
  -H "Content-Type: application/json" \
  -d '{
    "token": "mid_at_xxxxx",
    "client_id": "YOUR_CLIENT_ID",
...
app.get("/connect", (req, res) => {
  ...
  const authUrl = new URL("https://vendcfo.vercel.app/oauth/authorize");
...
app.get("/callback", async (req, res) => {
  ...
  const response = await fetch("https://api.vendhub.com/v1/oauth/token", {
    method: "POST",
...
app.get("/transactions", async (req, res) => {
  ...
  const result = await vendhub.transactions.list({
    pageSize: 50,
  });

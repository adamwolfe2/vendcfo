...
export async function GET(request: NextRequest) {
  try {
    ...
    const releaseResponse = await fetch(
      "https://api.github.com/repos/vendhub-ai/vendhub/releases/latest",
      {
        headers: {
          "User-Agent": "VendCFO-Desktop-Downloader",
    ...
    // Extract version number from tag (remove 'vendhub-v' prefix)
    const version = tag_name.replace(/^vendhub-v?/, "");

    // Construct DMG filename based on version and platform
    const filename = `VendCFO_${version}_${platform}.dmg`;

    // Construct download URL using the full tag name
    const downloadUrl = `https://github.com/vendhub-ai/vendhub/releases/download/${tag_name}/${filename}`;

...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Best ${competitor.name} Alternative for Founders (${year}) | VendCFO`;
  const description = `Looking for a ${competitor.name} alternative? Switch to VendCFO - built for founders, not accountants. Compare features, pricing, and see why teams are making the switch. Free trial available.`;
  ...
  return {
    ...
    keywords: [
      ...
      `${competitor.name.toLowerCase()} vs vendhub`,
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `VendCFO vs ${competitor.name} comparison`,
        },
      ],
    },
    twitter: {
...
export default async function Page({ params }: Props) {
  ...
  const webPageJsonLd = {
    ...
    description: `Looking for a ${competitor.name} alternative? Switch to VendCFO - built for founders, not accountants.`,

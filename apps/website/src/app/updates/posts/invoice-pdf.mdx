...
summary: "Discover how we're implementing PDF invoice generation in VendHub, using React-based solutions for customizable, efficient document creation."
...
<br />
We are building a new experience for invoices in VendHub. You'll be able to create and send invoices to your customers, as well as generate PDFs for each invoice. 
...
The invoice is then generated and saved to your [Vault](https://vendhub.com/vault), so we can match it to incoming transactions and mark it as paid. 
...
```tsx
import { InvoiceTemplate, renderToStream } from "@vendhub/invoice";
import { getInvoiceQuery } from "@vendhub/supabase/queries";
...
You can find the full code for the invoice template [here](https://go.vendhub.com/inv).
...
const CDN_URL = "https://cdn.vendhub.com";
